document.addEventListener('DOMContentLoaded', () => {
        ...
        loanForm.addEventListener('submit', async (e) => {
            ...
            if (dados.livro_id != 0 || dados.usuario_id != 0) {
                try {
                    ...
                    if (response.ok) {
                        // Diminui a quantidade — o backend da rota POST não faz isso automaticamente
                        await fetch(`${API_URL}/livro/${dados.livro_id}/diminuir`, { method: 'PUT' });
    ...
    window.editarEmprestimo = async (id_emprestimo) => {
        try {
            ...
            const leitor = document.getElementById('edit_leitor');
            const livro = document.getElementById('edit_livro');
            await carregarLeitores(leitor);
            await carregarLivros(livro);

            // Seleciona os valores corretos após carregar os selects
            leitor.value = emprestimo.id_usuario || emprestimo.usuario_id;
            livro.value = emprestimo.id_livro;

            document.getElementById('edit_data_emprestimo').value = emprestimo.data_emprestimo?.split('T')[0];
            document.getElementById('edit_data_prevista').value = emprestimo.data_devolucao_prevista?.split('T')[0];
            document.getElementById('edit_data_real').value = emprestimo.data_devolucao_real ? emprestimo.data_devolucao_real.split('T')[0] : '';
    ...
    // ─── Salvar edição do empréstimo ───────────────────────────────────────────
    // A lógica de aumentar/diminuir quantidade agora fica 100% no backend (PUT /emprestimo/:id)
    if (editLoanForm) {
    ...
    window.deletarEmprestimo = async (id_emprestimo) => {
        ...
        try {
            const response = await fetch(`${API_URL}/emprestimo/${id_emprestimo}`, { method: 'DELETE' });
            if (response.ok) {
                await carregarEmprestimos();
            } else {
